...
class Article extends ActiveRecord
{
   ...
   public function findById($id, $sectionId=''){
        //echo $sectionId;
        $cacheKey='EJinsightArticle__'.$id;
        ...
        if($article==false){
            ...
            if($article){
                ...
                //$article->seoTags = $article['seoTags'];
   ...
   public function findHotArticles($sectionId='', $limit, $interval){
    ...
    public function findRelatedByHashtags($articleId, $tags, $limit){
        ...
        $cacheKey='Article_findRelatedByHashtags_'.md5($articleId.$tags);

        $cache = Yii::$app->cache;
        $excludeSections=array();
        $relatedArticles=$cache->get($cacheKey);
        if($relatedArticles==false){
            $sql="SELECT DISTINCT `id` AS articleId FROM v_active_article v
        INNER JOIN eji.article_tag ar
        ON ar.articleId =v.id
        WHERE ar.articleId !='".$articleId."' AND (";
            //for($i=0;$i<count($tags);$i++) {
            for($i=0;$i<count($hashtag);$i++) {
                $t=$hashtag[$i];
                $sql .="ar.tag = '$t'";
                if ($i!=count($hashtag)-1) {
                    $sql .=" OR ";
                }
            }
            $sql .=") ORDER BY `publishDateLite` desc limit $limit";
            //echo $sql;
    
            //$related=app()->db->createCommand($sql)->queryAll();
            ...
            foreach($related as $r){
                ...
                $a=Article::findById($r['articleId']);

                if(!in_array($a->firstSection, $excludeSections) && Article::checkAvailable($a))
                    $relatedArticles[]=$a;
            }
            //app()->cache->set($cacheKey, $this->relatedArticles, 3600);

            if($relatedArticles){
                $cache->set($cacheKey, $relatedArticles, 3600);
            }

        }
        // var_dump($relatedArticles);
        return $relatedArticles;
    }

    public function checkAvailable($article){
        if(!$article)
            return false;
        // check status
        $t=date('Y-m-d H:i:s');
        if($article->status !=1 || $article->publishDate > $t)
            return false;
        if( $article->expiryDate != '0000-00-00 00:00:00' && $article->expiryDate < $t )
            return false;
        return true;
    }
   ...
   public function getSection($catId=''){
    ...
    public function hasVideo(){
        if($this->firstVideo != ''){
            ...
            $photos = Photo::find()->where(['articleId'=>$this->id, 'status'=> '1'])->orderBy(['photoOrder'=>SORT_ASC])->all();
            foreach($photos as $p){
                if($p->attachmentType =='VIDEO')
                    return $p;
            }
            //echo 'video';
            return true;
