...
class FeaturesController extends Controller
{
	...
	public function actionArticle($q, $suid)
	{
		...
		foreach ($topicmeta as $t){
			...
			$titles =FeaturesArticle::getDropDownList($q);

			return $this->render($view, 
				//compact('article')
						compact('article','topicmeta','label','titles')
			);
		}
	...
	{
		$cat=isset($code)? $code: '';
		Yii::$app->session->set('primarySection', $cat);
	...
	public function actionEvent()
	{
		...
		$list = Article::findAllBySection($sectionId, $limit, $page, $total, $range);
			...
			foreach($list as $key=>$arr)
			{
           	...
           	$this->view->params['trackEvent']=array(
				'category'=> '專題',
				'action'=> 'Features Listing',
				'label'=> 'FE:'.$cat_label.'|PG:1',
			);

           	return $this->render('category_events_combine', ['specials'=>$arr_output, 'cat'=>$cat]);
    }
	...
	public function actionTopic($tag)
	{
		...
		foreach  ($topicmeta2 as $v) {
			...
			if (strlen($v->abstract2)>20) { //$v->abstract2 is properly input
				...
				$this->redirect(Yii::$app->params['mobilewebUrl'].'/features/topicsp/tag/'.urlencode($tag));
			} else {	
...
/*			if ($v->abstract2=='') {
			...
			if (strlen($v->abstract2)<20) { //$v->abstract2 is empty or only a few characters
				//throw new CHttpException(404,'The requested page does not exist.');
				$this->redirect(Yii::$app->params['www1Urlnoslash'].'/features/topic/tag/'.urlencode($tag));
			} else {
				$rows = explode("\n", $v->abstract2);
				...
				function sortByOrder($a, $b) {
					return $a['OrderDesktop'] - $b['OrderDesktop'];
				}
				...
				for ($y = 0; $y <= count($widgetArray); $y++) {
					if ($widgetArray[$y]['OrderDesktop']==0){
						unset($widgetArray[$y]);
					}
				}
   	...
   	public function beforeAction($action)
	{
		...
		if ($action_id=='embedevent' || $action_id=='embedfeatures') { //no redirect for emebed widgets
			...
			if ($detect->isMobile() && !$detect->isTablet()) {	
	            ...
	            $mobileURL = preg_replace("%features%", 'featuresmob', $mobileURL);
		        ...
		        $fbarticleUrl = Yii::$app->params['www1Url'].'/'.Yii::$app->controller->id.'/'.$action_id;
