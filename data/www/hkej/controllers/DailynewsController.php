...
class DailynewsController extends Controller
{
    ...
    public $title='';
    	...
    	public function actionArticle($id)
		{
			...
			if ($article) {
                ...
                if ($_SERVER['HTTP_HOST'] == 'www1.hkej.com') {
				...
				if (preg_match('/dailynews\/article/', $_SERVER["REQUEST_URI"]) ) {
					...
					$mobileURL = str_replace("dailynews/article?id=", 'landing/mobarticle2/id/', $mobileURL);
			...
			Yii::$app->view->registerMetaTag(['property' => 'og:url', 'content' => Yii::$app->params['www1Urlnoslash'].Dailynews::genArticleUrl($article)], 'og_url');
			...
			if (($i=='I8') || ($i=='I9')) { 
				$sports=Article::findAllByMasterCat($dnewsToday, 'I8');
				$culture=Article::findAllByMasterCat($dnewsToday, 'I9');
				$tmp=array_merge($culture, $sports);
				//$articles=$tmp;
				$articles=Dailynews::sortByMsort($tmp);
			...
			*/
				...
				if ($article->author) {
					//update author view count
					if ($_SERVER['HTTP_HOST'] == 'www1.hkej.com') {
						Author::updateViewCnt($article->authorId);
					}
	...
	public function actionArticleprint($id)
	{
	    ...
	    public function actionCntw()
		{
			...
			if(isset($_REQUEST['date']) && !empty($_REQUEST['date'])){
				...
				if (DailyNews::isPubDate($pub_date)==false) {
					 return $this->render('list_empty', ['pub_date'=>$pub_date]);
				}
			} else {
		...
		public function actionCulture()
		{
			...
			if (file_exists($listing_json) ==0) {
		   	...
		   	//read json
				...
				$articles1 = EjHelper::mcat_filter($i, $articles);
				...
				$articles2 = EjHelper::mcat_filter($i, $articles);
		...
		public function actionGentoc()
		{
			...
			if ($pub_date <= date("Y-m-d")) {

				//$pub_date = Yii::$app->session->get('dnewsToday');
				$yyyy = date('Y', strtotime($pub_date));
				$pub_date2 = str_replace('-', '', $pub_date);

				$listing_json = Yii::getAlias('@app').'/assets/dailynewsWeb/'.$yyyy.'/hkej_dailynews_'.$pub_date2.'_toc.json';
				$file = $listing_json;
				
				if ((DailyNews::isPubDate($pub_date)==false)){
					// create empty json
					$json['articles'] = "NOT_PUB_DATE";
		...
		public function actionToparticlesjson() {
			$cacheKey='dailynewsMostPopular';
			$cache = Yii::$app->cache;
			$articles=$cache->get($cacheKey);

			$latestPubDate=DailyNews::getLatestPubDate();
			
			if($articles==false){
				/*$sql="SELECT ar. *
				FROM  `v_active_article` ar
				WHERE  `catId` =1009
				AND publishDateLite =  '$latestPubDate'
				ORDER BY viewCnt DESC
				LIMIT 15";*/
				$limit=15;
				$articles=Dailynews::findDNewsHotArticles($limit);
				if(count($articles) > 0){
					// 120 seconds
					$cache->set($cacheKey, $articles, 120);
				// one more set in File Cache
					//cache->set($fileCache, $articles, 120);
				}
				}
				...
				foreach ($articles as $a) {
					...
					$tmp['catid']=$a->catId;
					$tmp['publish_date']=$latestPubDate;				
					$tmp['author_name']=$authorName;
					$tmp['storyProgName']=$a->storyProgName;
					if ($a->firstPhoto != '') {
				...
				exit;			
				//return $this->renderPartial('top_articles', array('articles'=>$articles, 'latestPubDate'=>$latestPubDate));
		}	
    	...
    	public function beforeAction($action)
		{
			...
			if (($action_id != 'getdays') && ($action_id != 'toc') && ($action_id != 'index') && ($action_id != 'article') && ($action_id != 'author') && ($action_id != 'gentoc') && ($action_id != 'getkeynews') && ($action_id != 'toparticlesjson') && $detect->isMobile() && !$detect->isTablet()) {
				$mobileURL = Yii::$app->params['mobilewebUrl'].$_SERVER["REQUEST_URI"];
				...
				if(($action_id != 'getdays') && ($action_id != 'gentoc') && ($action_id != 'getkeynews') && ($action_id != 'index') && ($action_id != 'article') && ($action_id != 'toparticlesjson')){
