...
class WebFeaturesSideWidget extends Widget
{	
	...
	{
		$articles=[];
		$cacheKey='features_widget_side_'.$tag.'_'.$page;
		$cache=Yii::$app->cache;
		$articles=$cache->get($cacheKey);

		if($articles == false){
			$data=[];
			$url= Yii::$app->params['featuresLandingUrl'];
    ...
    public function run()
    {
        return $this->render('web_feature_side_widget', ['stickies'=>$this->stickies]);
    }
}
